...
class ApiService {
  ...
  Future<Map<String, dynamic>> getProductDetails(String productId) async {
    try {
      ...
      // 直接構建完整的 URL
      String url =
          '$_baseUrl/gws_appproduct&api_key=$_apiKey&product_id=$productId';
  ...
  Future<Map<String, dynamic>> searchProducts(String keyword) async {
    try {
      final userId = await _getCustomerId();
      final url =
          '$_baseUrl/gws_appproducts/search&search=${Uri.encodeComponent(keyword)}&api_key=$_apiKey';
      final finalUrl = userId != null ? '$url&customer_id=$userId' : url;
      ...
      if (response.data is Map) {
        ...
        throw Exception('無效的響應格式');
      }
    } catch (e) {
      throw Exception('搜尋產品時發生錯誤: $e');
    }
  }
  ...
  Future<Map<String, dynamic>> getEcpaySettings() async {
    try {
      ...
      // 如果沒有找到設置，使用測試環境的默認值
      if (!ecpaySettings.containsKey('merchant_id') ||
          ecpaySettings['merchant_id'] == null) {
        ecpaySettings['merchant_id'] = '2000132';
        debugPrint('使用測試環境默認 merchant_id: 2000132');
      }

      if (!ecpaySettings.containsKey('hash_key') ||
          ecpaySettings['hash_key'] == null) {
        ecpaySettings['hash_key'] = '5294y06JbISpM5x9';
        debugPrint('使用測試環境默認 hash_key: 5294y06JbISpM5x9');
      }

      if (!ecpaySettings.containsKey('hash_iv') ||
          ecpaySettings['hash_iv'] == null) {
        ecpaySettings['hash_iv'] = 'v77hoKGq4kWxNNIS';
        debugPrint('使用測試環境默認 hash_iv: v77hoKGq4kWxNNIS');
      }

      // 添加支付方式
      ecpaySettings['payment_methods'] =
          response['payment_ecpaypayment_payment_methods'] ?? 'Credit';
  ...
  Future<Map<String, dynamic>> getZones(String countryId) async {
    try {
      final response = await _get(
        'gws_zone',
        extraParams: {'country_id': countryId},
      );
      return response;
    } catch (e) {
      throw Exception('API 請求錯誤: ${e.toString()}');
  ...
  Future<Map<String, dynamic>> register(Map<String, dynamic> userData) async {
    try {
      ...
      final registerUrl = 'gws_appcustomer/add';
      ...
      if (response.statusCode == 200) {
            ...
            if (responseStr.trim().startsWith('{') ||
              ...
              if (responseStr.toLowerCase().contains('success') ||
                ...
                return {
                  'success': true,
                  'message': [
                    {'msg': responseStr, 'msg_status': true},
                  ],
                };
              }
            }
        ...
        return {
          'error': true,
          'message': [
            {'msg': '請求失敗: ${response.statusCode}', 'msg_status': false},
          ],
        };
      }
      ...
      return <String, dynamic>{
        'error': true,
        'message': [
          {'msg': '發生錯誤: ${e.toString()}', 'msg_status': false},
        ],
      };
    }
  }
  ...
  Future<Map<String, dynamic>> addToWishlist(
    ...
    return _get(
      'gws_appcustomer_wishlist/add',
      extraParams: {'customer_id': customerId, 'product_id': productId},
    );
  }
  ...
  Future<Map<String, dynamic>> removeFromWishlist(
    ...
    return _get(
      'gws_appcustomer_wishlist/remove',
      extraParams: {'customer_id': customerId, 'product_id': productId},
    );
  }
  ...
  Future<Map<String, dynamic>> getInformationById(String informationId) async {
    try {
      final response = await _get(
        'gws_information&information_id=$informationId',
      );
      return response;
    } catch (e) {
      rethrow;
  ...
  Future<Map<String, dynamic>> getCustomerProfile() async {
    try {
      ...
      final response = await _get('gws_appcustomer&customer_id=$customerId');
      return response;
    } catch (e) {
      rethrow;
    }
  ...
  Future<Map<String, dynamic>> getCustomerAddress(String addressId) async {
    try {
      ...
      final response = await _get(
        'gws_appcustomer_address&customer_id=$customerId&address_id=$addressId',
      );
      return response;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getCustomerAddressList(String customerId) async {
    try {
      ...
      final response = await _get(
        'gws_appcustomer_address&customer_id=$customerId',
      );
      return response;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> addCustomerAddress(
    ...
    try {
      ...
      // 構建 URL
      final url =
          '$_baseUrl/gws_appcustomer_address/add&api_key=$_apiKey&customer_id=$customerId';
  ...
  // 修改客戶地址
  Future<Map<String, dynamic>> editCustomerAddress(
    String addressId,
    Map<String, dynamic> addressData,
  ) async {
    try {
      ...
      final url =
          '$_baseUrl/gws_appcustomer_address/edit&customer_id=$customerId&address_id=$addressId&api_key=$_apiKey';
  ...
  Future<Map<String, dynamic>> deleteCustomerAddress(
    ...
    try {
      ...
      // 使用 GET 方法刪除地址
      return _get(
        'gws_appcustomer_address/remove',
        extraParams: {'customer_id': customerId, 'address_id': addressId},
      );
    } catch (e) {
  ...
  Future<Map<String, dynamic>> updateCustomerProfile(
    ...
    try {
      // 構建 URL
      final url =
          '$_baseUrl/gws_appcustomer/edit&api_key=$_apiKey&customer_id=$customerId';
  ...
  Future<Map<String, dynamic>> addToCart({
    ...
    try {
      ...
      // 構建 URL
      final url =
          '$_baseUrl/gws_appcustomer_cart/add&customer_id=$customerId&api_key=$_apiKey';
  ...
  Future<Map<String, dynamic>> updateCartQuantity(
    ...
    try {
      ...
      // 構建 URL
      final url =
          '$_baseUrl/gws_appcustomer_cart/edit&customer_id=$customerId&cart_id=$cartId&api_key=$_apiKey';

      // 發送 POST 請求，包含數量參數
      final formData = FormData.fromMap({'quantity': quantity.toString()});
  ...
  Future<Map<String, dynamic>> removeFromCart(String cartId) async {
    try {
      ...
      // 構建 URL - 使用正確的 API 路徑
      final url =
          '$_baseUrl/gws_appcustomer_cart/remove&customer_id=$customerId&cart_id=$cartId&api_key=$_apiKey';
  ...
  Future<Map<String, dynamic>> getOrderDetail(
    ...
    try {
      ...
      final response = await _get(
        'gws_appcustomer_order/info',
        extraParams: {'customer_id': customerId, 'order_id': orderId},
      );
      return response;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> createOrder(
    ...
    try {
      ...
      // 構建 URL
      final url =
          '$_baseUrl/gws_appcustomer_order/add&customer_id=$customerId&api_key=$_apiKey';
      ...
      if (response.statusCode == 200) {
        ...
        return {
          ...
          'message': [
            {'msg': errorMessage, 'msg_status': false},
          ],
        };
      }
    } catch (e) {
      ...
      if (e is DioException) {
        ...
        return {
          ...
          'message': [
            {'msg': '訂單提交失敗: ${e.toString()}', 'msg_status': false},
          ],
        };
      }
  ...
  Future<void> clearCart(String customerId) async {
    try {
      // 先獲取購物車列表
      final cartResponse = await _get(
        'gws_appcustomer_cart',
        extraParams: {'customer_id': customerId},
      );

      if (cartResponse.containsKey('customer_cart') &&
          cartResponse['customer_cart'] is List) {
        final cartItems = List<Map<String, dynamic>>.from(
          cartResponse['customer_cart'],
        );
        ...
        for (var item in cartItems) {
          ...
          if (cartId != null) {
            // 使用正確的 API 路徑和參數
            await _get(
              'gws_appcustomer_cart/remove',
              extraParams: {'customer_id': customerId, 'cart_id': cartId},
            );
          }
        }
      }
